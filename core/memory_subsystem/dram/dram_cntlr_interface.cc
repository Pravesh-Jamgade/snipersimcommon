...
void  DramCntlrInterface::loggingDRAM(IntPtr addr, Core::mem_op_t mem_op)
{
   ...
   if((getMemLevelDebug()!="" && getMemLevelDebug()==getName() ) || getMemLevelDebug()=="")
   {
      ...
      totalAccess++;
      SInt16 statusBits = 0;
      cache_helper::Misc::PiggyBackStatus(statusBits,getName(),typeAccess,false);
      getCacheHelper()->addRequest(eip, addr, getName(), NULL, cycleCount, typeAccess, statusBits);
   }
}